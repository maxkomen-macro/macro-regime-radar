...
_Z_LABELS = {
    ...
    "DGS2_weekly_chg_z":   "2Y Treasury yield",
...
_Z_TO_RAW = {
    ...
    "DGS2_weekly_chg_z":   "DGS2_weekly_chg",
...
def render_surprises(dm: pd.DataFrame, top_n: int = 10, title: str = "Top Surprises This Week") -> None:
    ...
    for i, row in surprises.iterrows():
        ...
        bar_color = "#e05252" if abs(z) >= 2.5 else ("#e0812f" if abs(z) >= 1.5 else "#7fa6e0")
        st.markdown(
            f"""<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
  <span style="font-size:16px">{icon}</span>
  <div style="flex:1">
    <div style="font-size:13px;line-height:1.4">{row['interpretation']}</div>
    <div style="height:4px;border-radius:2px;background:#172561;margin-top:3px">
