...
LEVEL_COLORS = {"risk": "#d23f3f", "watch": "#b8860b", "info": "#000b3d"}
...
def render_alerts_tab() -> None:
    ...
    for _, row in filtered.head(50).iterrows():
        ...
        color  = LEVEL_COLORS.get(lvl, "#5b6480")
        ...
        val_html = (
            f'<div style="font-size:11px;color:#8a92a8;margin-top:3px">{val_line}</div>'
            if val_line else ""
        )

        st.markdown(
            f'<div style="border-left:3px solid {color};background:#ffffff;'
            f'border:0.5px solid #e3e6ec;border-left:3px solid {color};'
            ...
            f'<br><span style="font-weight:700;font-size:14px;color:#0b1540">{display_name}</span>'
            f'</div>'
            f'<span style="font-size:11px;color:#5b6480;white-space:nowrap;padding-left:8px">{dt_str}</span>'
            f'</div>'
            f'<div style="font-size:13px;margin-top:5px;color:#2c3556">{why_it_matters}</div>'
            f'{val_html}'
            f'</div>',
