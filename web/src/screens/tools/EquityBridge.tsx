...
const BAR_W = 44.9;
const BASE_Y = 228;
/** The tallest bar's height (mockup: exit equity at 168.8 px). */
const PLOT_H = 168.8;
const STUB_H = 2;
const LABEL_Y = [243, 255] as const;
...
export default function EquityBridge({ steps }: { steps: BridgeStep[] }) {
  ...
  const scale = maxValue > 0 ? PLOT_H / maxValue : 0;
  const y = (v: number): number => BASE_Y - v * scale;
  ...
  return (
    ...
    >
      ...
      <line data-role="baseline" x1={f1(X0 - 4)} x2={f1(X0 + last * PITCH + BAR_W + 4)} y1={BASE_Y} y2={BASE_Y} stroke="rgba(255,255,255,.18)" />
      {bars.map((b, i) => (
        <g key={i} data-role="step">
          <rect data-role="bar" x={f1(b.x)} y={f1(b.top)} width={f1(BAR_W)} height={f1(b.height)} rx="3" fill={b.fill} fillOpacity={b.opacity} />
          <text
            data-role="value"
            x={f1(b.cx)}
            y={f1(b.top - 6)}
          ...
          {steps[i].label.map((line, li) =>
            line ? (
              <text key={li} data-role="label" x={f1(b.cx)} y={LABEL_Y[li]} textAnchor="middle" fontSize="10.5" fill="var(--text-3)" style={{ fontFamily: "var(--font-ui)" }}>
                {line}
              </text>
            ) : null,
          )}
