...
## RAW
Source rows as fetched, one provider per series, not rewritten after the fetch.
...
- `desk_series` · the Desk's daily history per series (FRED direct, merged so a rolling window keeps every observation it was once served; tier 2 adds market closes), read by the event study.
- `ai_spend_ledger` · append-only cost of every AI call, kept in full.
- `event_calendar` · the hand-maintained macro calendar.

## CUR
Curated series the analytics read, at the RAW grain, checked by `scripts/validate_db.py` (integrity, counts, max-date regression, freshness verdicts) before a release is published.
...
## MART
Computed output, one row per date, read by the API and this Desk. One number, one truth: the browser does not re-derive these.
