...
- The assistant is built (2026-08-26): `POST /api/assistant/ask` streams SSE from
  ...
  copied [repo].
- The browser stores two per-visitor preferences, both in localStorage: the watchlist (mrr.watchlist.v1: add, remove, reorder) and the sidebar's collapsed state (mrr.sidebar.v1). No accounts, no server storage. The assistant's no-persistence rule is unchanged.
